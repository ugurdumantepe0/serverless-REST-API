...
module.exports.sortByDifficulty = (event, context, callback) => {
  // get all challenges from the database
  dynamoDb.scan(params, (error, result) => {
    // do ascending order
    if (event.pathParameters.sortOrder === 'ascending') {
        sortedChallenges = helper.sortChallengesByDifficulty(result.Items, 'asc');
    }
    // do descending order 
    else if (event.pathParameters.sortOrder === 'descending') {
        sortedChallenges = helper.sortChallengesByDifficulty(result.Items, 'des');
    }
    else {
