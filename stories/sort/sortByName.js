...
module.exports.sortByName = (event, context, callback) => {
  // get all challenges from the database
  dynamoDb.scan(params, (error, result) => {
    // do ascending order
    if (event.pathParameters.sortOrder === 'ascending') {
        sortedChallenges = helper.sortChallenges(result.Items, 'name', 'asc');
    }
    // do descending order 
    else if (event.pathParameters.sortOrder === 'descending') {
        sortedChallenges = helper.sortChallenges(result.Items, 'name', 'des');
    }
    else {
